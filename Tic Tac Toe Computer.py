...
class Game(Tk):
    ...
    def click(self, event):
        ...
        #duplication /!\
        elif (self.gamestate == STATE_O_TURN or self.gamestate == STATE_X_TURN and
    ...
    def new_move(self, player, grid_x, grid_y):
        ...
        """
        bir = random.randint(0, 2)
        iki = random.randint(0, 2)
        #duplication /!\
        dur=0
        if player == X:
            while dur==0:
                if self.board[iki][bir] == EMPTY:
                    self.draw_X(bir, iki)
                    self.board[iki][bir] = X
                    dur=1
                else:
                    bir = random.randint(0, 2)
                    iki = random.randint(0, 2)
